...
class BoardDetail extends Component {
  ...
  subscribeOrUnsubscribe = slug => {
    axios
      .get(`/api/frontboard/actions/subscribe/?board_slug=${slug}`)
      .then(res => {
          ...
          <div className="card-body" style={{ padding: 10 }}>
            <div style={{ float: "left", width: "80%" }}>
              ...
              <p className="text-muted">
                <span title="c">{board.subscribers_count} subscribers</span>,
                created
                <span>{board.created_naturaltime}</span> •{" "}
                <Link to="/report" className="text-muted">
                  report
                </Link>
              </p>
            </div>
            <div
              ...
              {board.is_subscribed ? (
                <button
                  ...
                  onClick={() => this.subscribeOrUnsubscribe(board.slug)}>
                  Unsubscribe
                </button>
              ) : (
                <button
